...
df <- read.csv("logger_One.csv", skip = 1, header = TRUE, check.names = FALSE)
...
# Time filter
start_time <- ymd_hms("2025-11-09 11:00:00")
end_time   <- ymd_hms("2025-11-09 14:00:00")
...
ggsave("temperature_plot_logger1.png", width = 10, height = 6, dpi = 300)
#ggsave("temperature_plot_logger1.pdf", width = 10, height = 6)
...
plot_A <- df_long %>%
  ...
  labs(
    title = "Burn Temperature - Sensor A (Logger One)",
    x = "Date/Time",
    y = "Temperature (°F)"
  ) +
  theme_minimal()+
...
plot_B <- df_long %>%
  ...
  labs(
    title = "Burn Temperature - Sensor B (Logger One)",
    x = "Date/Time",
    y = "Temperature (°F)"
  ) +
  theme_minimal()+
...
plot_C <- df_long %>%
  ...
  labs(
    title = "Burn Temperature - Sensor C (Logger One)",
    x = "Date/Time",
    y = "Temperature (°F)"
  ) +
  theme_minimal()+
...
plot_D <- df_long %>%
  ...
  labs(
    title = "Burn Temperature - Sensor D (Logger One)",
    x = "Date/Time",
    y = "Temperature (°F)"
  ) +
  theme_minimal()+
...
# Optional: If you want to arrange all four plots in a grid, install and use patchwork or gridExtra
library(patchwork)
(plot_A | plot_B) / (plot_C | plot_D)
ggsave("separate_temp_plots_logger1.png", width = 10, height = 6, dpi = 300)

#plot for the entire duration of time series
# ---- Plot all four sensors (full time series) ----
...
ggplot(df_long_full, aes(x = DateTime, y = Temperature, color = Sensor)) +
  geom_line(size = 1) +
  labs(
    title = "Burn Temperature - Full Time Series (Logger One)",
    x = "Date/Time",
    y = "Temperature (°F)",
    color = "Sensor"
  ) +
...
ggsave("full_temp_plots_logger1.png", width = 10, height = 6, dpi = 300)
