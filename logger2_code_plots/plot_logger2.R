# logger 2 data viz
# burn occurred 11/8/25
# thermocouple at BDH was dislogdged during burn
...
df <- read.csv("logger_two_20251109.csv", skip = 1, header = TRUE, check.names = FALSE)
...
ggsave("full_temp_plots_logger2.png", width = 10, height = 6, dpi = 300)

# Time filter -- set based on looking at where burn starts
start_time <- ymd_hms("2025-11-08 14:45:00")
end_time   <- ymd_hms("2025-11-08 16:30:00")
...
ggsave("temperature_plot_logger2.png", width = 10, height = 6, dpi = 300)
ggsave("temperature_plot_logger2.pdf", width = 10, height = 6)
...
plot_A <- df_long %>%
  ...
  labs(
    title = "Burn Temperature - Sensor A (Logger 2)",
    x = "Date/Time",
    y = "Temperature (°F)"
  ) +
  theme_minimal()+
...
plot_B <- df_long %>%
  ...
  labs(
    title = "Burn Temperature - Sensor B (Logger 2)",
    x = "Date/Time",
    y = "Temperature (°F)"
  ) +
  theme_minimal()+
...
plot_C <- df_long %>%
  ...
  labs(
    title = "Burn Temperature - Sensor C (Logger 2)",
    x = "Date/Time",
    y = "Temperature (°F)"
  ) +
  theme_minimal()+
...
plot_D <- df_long %>%
  ...
  labs(
    title = "Burn Temperature - Sensor D (Logger 2)",
    x = "Date/Time",
    y = "Temperature (°F)"
  ) +
  theme_minimal()+
...
ggsave("separate_temp_plots_logger2.png", width = 10, height = 6, dpi = 300)